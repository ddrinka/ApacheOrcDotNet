...
namespace ApacheOrcDotNet.OptimizedReader.Buffers
{
    public class DecimalAsDoubleColumnBuffer : BaseColumnBuffer<double>
    {
        ...
        public DecimalAsDoubleColumnBuffer(IByteRangeProvider byteRangeProvider, OrcFileProperties context, OrcColumn column) : base(byteRangeProvider, context, column)
        {
        ...
        public override void Fill()
        {
            ...
            if (presentValuesRead > 0)
            {
                ...
                for (int idx = 0; idx < presentValuesRead; idx++)
                {
                    ...
                    {
                        _values[_numValuesRead++] = BigIntegerToDouble(_dataStreamValues[valueIndex], _secondaryStreamValues[valueIndex]);
                        valueIndex++;
                    }
                    else

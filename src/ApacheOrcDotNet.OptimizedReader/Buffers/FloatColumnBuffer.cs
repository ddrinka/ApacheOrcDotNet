...
namespace ApacheOrcDotNet.OptimizedReader.Buffers
{
    public class FloatColumnBuffer : BaseColumnBuffer<float>
    {
        ...
        public FloatColumnBuffer(IByteRangeProvider byteRangeProvider, OrcContextNew context, OrcColumn column) : base(byteRangeProvider, context, column)
        {
                ...
                if (presentStream != null)
                {
                    for (int idx = 0; idx < numPresentValuesRead; idx++)
                    {
                        if (_presentStreamBuffer[idx])
                        {
                            ...
                            _values[_numValuesRead++] = BitConverter.ToSingle(valueBuffer);

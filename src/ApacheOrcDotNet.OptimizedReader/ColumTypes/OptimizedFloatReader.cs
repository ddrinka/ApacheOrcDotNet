...
{
    public class OptimizedFloatReader : BaseColumnReader<float>
    {
        public OptimizedFloatReader(ReaderContext readerContext) : base(readerContext)
        {
        }
                    ...
                    if (presentStream != null)
                    {
                        for (int idx = 0; idx < numPresentValuesRead; idx++)
                        {
                            if (presentBuffer[idx])
                            {
                                ...
                                _outputValuesRaw[_numValuesRead++] = BitConverter.ToSingle(valueBuffer);
                                dataReader.Advance(valueBuffer.Length);
                            }
                            else
                                _outputValuesRaw[_numValuesRead++] = float.NaN;
                        }
                    }
                    else
                    { 
